...
%macro loess (VOI, smooth);
%let p =p%sysfunc(tranwrd(&VOI,%str( ),%str(p)));
proc loess data=Results.LC6Sorting plots=none;
  model  &VOI  =TIMESTAMP/smooth = &smooth;
score;
ods output ScoreResults= &p  (drop = smoothingparameter scoredata);
run;
%mend;

%loess ( LCClean1_6, 0.0005 );

%loess ( LCClean2_6, 0.0005 );
...
%macro loess (VOI, smooth);
%let p =maxp%sysfunc(tranwrd(&VOI,%str( ),%str(maxp)));
proc loess data=Results.LC6Sorting plots=none;
  model  &VOI  =TIMESTAMP/smooth = &smooth;
score;
ods output ScoreResults= &p  (drop = smoothingparameter scoredata);
...
%loess ( LCClean1_6, 0.0555 );

%loess ( LCClean2_6, 0.0555 );
/*proc loess data=Results.LC6Sorting plots=none;*/
...
data Results.LC6LoessResults ;
  merge pLCClean1_6 pLCClean2_6;
  by TIMESTAMP;
run;

data Results.maxLC6LoessResults ;
  merge maxpLCClean1_6 maxpLCClean2_6;
  by TIMESTAMP;
run;
...
proc transpose data = Results.maxLC6LoessResults 
               out = Results.maxLC6LoessTransResults 
               prefix = obs;
  var p_LCClean1_6 p_LCClean2_6;
  id obs;
run;
...
/*Locate the extrema*/
data Results.maxLC6Extrema (keep = obs type _NAME_);
  set Results.maxLC6LoessTransResults;
*Group observed values into array
  ;
  array aval[*] obs:;
    *buffer at each end of graph to avoid edge effects;
...
proc sort data = Results.maxLC6Extrema;
    by _NAME_;
run;
...
data Neighborhoodmax(drop = nbrhood d);
  *Only bring in records of the extrema type we want;
  set Results.maxLC6Extrema(where = (lowcase(type) = "max"));
  ...
  *Find records within the neighborhood;
  do d = -nbrhood to nbrhood;
    Rec = Obs + d;
    set Results.maxLC6LoessResults(drop = obs) point = Rec;
    output;
  end;
run;
...
data Results.LC6final(keep=Timestamp Date Time Light_dark_period LCClean1_6 LCClean2_6 CmaxLCClean1_6 CmaxLCClean2_6 minLCClean1_6 minLCClean2_6);
  set Irrproj.MASTERSASFILECLEAN (where=(timestamp < '08nov2020:10:25:00'dt))
      LC6after (where=(timestamp > '08nov2020:10:25:00'dt));
run;

proc sort data = Results.LC6final
          nodupkey ; 
  by TIMESTAMP;
run;
quit;
...
%macro plots (VOI);
%let min =min%sysfunc(tranwrd(&VOI,%str( ),%str(min)));
proc sgplot data=Results.LC6final(where=('08nov2020:10:25:00'dt <= TIMESTAMP <= '14nov2020:10:25:00'dt)) noautolegend;
